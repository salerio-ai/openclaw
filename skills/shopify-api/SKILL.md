...
`node skills/shopify-api/scripts/run.js ...`
...
```bash
node skills/shopify-api/scripts/run.js "mutation { shopUpdate(input: { name: \"New Name\" }) { shop { id name } userErrors { field message } } }"
```

### Use a query file + variables

```bash
node skills/shopify-api/scripts/run.js --file ./query.graphql --vars '{"first":10}'
```
