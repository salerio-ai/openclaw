...
`node skills/bustly-search-data/scripts/run.js ...`
...
```bash
node skills/bustly-search-data/scripts/run.js query "SELECT * FROM semantic.dm_orders_shopify WHERE created_at >= '2025-01-01' LIMIT 10"
```
...
Use `get_schema` first, then map columns manually in SQL by aliasing to common names.
Example: `total_inc_tax AS total_price`.
...
2. **Normalize in SQL** - alias different platform fields to common names (`AS total_price`, etc.)
