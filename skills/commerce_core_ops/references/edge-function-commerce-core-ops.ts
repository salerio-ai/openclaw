...
function normalizeOperation(value: unknown): NormalizedOperation {
  ...
  if (normalized === "create" || normalized === "insert" || normalized === "import") return "create";
  ...
  if (normalized === "inventory_adjust" || normalized === "inventory" || normalized === "stock_adjust") {
...
function normalizeReadEntity(value: unknown): ReadEntity {
  ...
  if (normalized === "shop_info" || normalized === "shop" || normalized === "store") return "shop_info";
...
function isActiveStatus(value: unknown): boolean {
  ...
  if (typeof value === "boolean") return value;
  const normalized = String(value).trim().toUpperCase();
  return normalized === "ACTIVE" || normalized === "ENABLED" || normalized === "1" || normalized === "TRUE";
}
...
async function resolveMagentoContext(
  ...
  if ((!magentoBaseUrl || !magentoStoreId) && magentoAccountId) {
    try {
      ...
      magentoBaseUrl = magentoBaseUrl || pickString(account.base_url, account.site_url, account.store_url);
...
function trimRows(rows: unknown, limit: number): Record<string, unknown>[] {
  ...
  return list.slice(0, limit).map((item) => (typeof item === "object" && item ? item as Record<string, unknown> : { value: item }));
}
...
async function callShopifyGraphQL(
  ...
  if (!response.ok) {
    throw new Error(`Shopify API ${response.status} ${response.statusText}: ${JSON.stringify(parsed)}`);
  }
...
async function readShopifyEntity(
  ...
  if (request.entity === "shop_info") {
    ...
    const rows = data.shop && typeof data.shop === "object" ? [data.shop as Record<string, unknown>] : [];
    return { provider: "shopify", entity: request.entity, count: rows.length, rows, pagination: null };
  }
  ...
  const variants = products.flatMap((product) =>
    trimRows((product.variants as Record<string, unknown> | undefined)?.nodes, 100).map((variant) => ({
...
function buildShopifyProductMutation(
  ...
  if (op === "create") {
    ...
    requireShopifyField(input, "title", "Shopify create requires payload.title or payload.input.title");
    return {
      query:
  ...
  if (op === "delete") {
    const input = (payload.input && typeof payload.input === "object"
      ? payload.input
      : { id: payload.id }) as Record<string, unknown>;
  ...
  if (op === "unpublish") {
    requireShopifyField(payload, "publicationId", "Shopify unpublish requires payload.publicationId");
  ...
  if (op === "variants_bulk_update") {
    requireShopifyField(payload, "productId", "Shopify variants_bulk_update requires payload.productId");
    requireShopifyField(payload, "variants", "Shopify variants_bulk_update requires payload.variants");
    return {
      query:
  ...
  if (op === "inventory_adjust") {
    requireShopifyField(payload, "locationId", "Shopify inventory_adjust requires payload.locationId");
...
async function writeShopifyProduct(
  ...
  const finalOperation =
    operation === "upsert"
      ? (pickString(payload.id, (payload.input as Record<string, unknown> | undefined)?.id) ? "update" : "create")
      : operation;
  ...
  const response = await callShopifyGraphQL(context, mutation.query, mutation.variables, apiVersion);
...
async function readBigCommerceEntity(
  ...
  if (request.entity === "products") {
    ...
    if (productId) {
      ...
      return { provider: "bigcommerce", entity: request.entity, count: row.length, rows: row, pagination: null };
    }
    ...
    const pagination = response.meta && typeof response.meta === "object"
  ...
  if (request.entity === "variants" || request.entity === "inventory") {
    ...
    if (variantId) {
      ...
      rows = response.data && typeof response.data === "object" ? [response.data as Record<string, unknown>] : [];
    } else {
  ...
  if (request.entity === "customers") {
    ...
    if (customerId) {
      ...
      const rows = trimRows(response.data, request.limit);
      return { provider: "bigcommerce", entity: request.entity, count: rows.length, rows, pagination: null };
    }
  ...
  if (orderId) {
    ...
    const rows = trimRows(response.data, request.limit).map((row) => ({ order_id: orderId, ...row }));
    return { provider: "bigcommerce", entity: request.entity, count: rows.length, rows, pagination: null };
  }
...
async function readWooCommerceEntity(
  ...
  if (request.entity === "shop_info") {
    ...
    return { provider: "woocommerce", entity: request.entity, count: 1, rows: [response], pagination: null };
  }
  ...
  if (request.entity === "products") {
    ...
    const rows = trimRows(response.data, request.limit);
    return { provider: "woocommerce", entity: request.entity, count: rows.length, rows, pagination: null };
  }
  ...
  if (request.entity === "variants") {
    ...
    if (explicitProductId) {
      ...
      const rows = trimRows(response.data, request.limit).map((row) => ({ product_id: explicitProductId, ...row }));
      return { provider: "woocommerce", entity: request.entity, count: rows.length, rows, pagination: null };
    }
    ...
    for (const product of products) {
      ...
      const variantRows = trimRows(response.data, remaining).map((row) => ({ product_id: productId, ...row }));
  ...
  if (orderId) {
    ...
    const rows = trimRows(order.line_items, request.limit).map((item) => ({ order_id: orderId, ...item }));
    return { provider: "woocommerce", entity: request.entity, count: rows.length, rows, pagination: null };
  }
...
async function readMagentoEntity(
  ...
  if (request.entity === "shop_info") {
    ...
    const rows = trimRows(response.data, request.limit);
    return { provider: "magento", entity: request.entity, count: rows.length, rows, pagination: null };
  }
  ...
  if (request.entity === "products") {
    ...
    if (sku) {
      ...
      return { provider: "magento", entity: request.entity, count: 1, rows: [response], pagination: null };
    }
  ...
  if (request.entity === "customers") {
    const customerId = pickString(request.filters.id, request.filters.customer_id);
    const query = buildMagentoSearchQuery(request.limit, request.since, "updated_at", "entity_id", customerId);
    const response = await callMagentoApi(baseUrl, accessToken, "GET", `/V1/customers/search?${query}`);
    const rows = trimRows(response.items, request.limit);
    return {
      provider: "magento",
      entity: request.entity,
  ...
  if (request.entity === "orders" || request.entity === "order_items") {
    const orderId = pickString(request.filters.id, request.filters.order_id);
    const query = buildMagentoSearchQuery(request.limit, request.since, "updated_at", "entity_id", orderId);
...
async function writeBigCommerceProduct(
  ...
  const finalOperation =
    operation === "upsert"
      ? (pickString(payload.id, payload.product_id) ? "update" : "create")
      : operation;

  if (finalOperation === "create") {
    const body = extractProductInput(payload);
    const response = await callBigCommerceApi(storeHash, accessToken, "POST", "/v3/catalog/products", body);
  ...
  if (finalOperation === "inventory_adjust") {
    ...
    let targetLevel = parseNumber(payload.inventory_level) ??
    ...
    if (!productId) {
      throw new Error("BigCommerce inventory_adjust requires payload.product_id or resolvable variant product_id");
    }
...
async function updateBigCommerceVariantInventory(
  ...
  const productVariantPath =
    `/v3/catalog/products/${encodeURIComponent(productId)}/variants/${encodeURIComponent(variantId)}`;
...
async function writeWooCommerceProduct(
  ...
  const finalOperation =
    operation === "upsert" ? (pickString(payload.id, payload.product_id) ? "update" : "create") : operation;
  ...
  if (finalOperation === "delete") {
    ...
    return { provider: "woocommerce", operation: finalOperation, product_id: productId, force, response };
  }

  if (finalOperation === "inventory_adjust") {
    const productId = pickString(payload.id, payload.product_id);
    if (!productId) throw new Error("WooCommerce inventory_adjust requires payload.id or payload.product_id");

    let targetQty = parseNumber(payload.stock_quantity) ??
...
async function writeMagentoProduct(
  ...
  if (finalOperation === "inventory_adjust") {
    ...
    let targetQty = parseNumber(payload.qty) ??
      parseNumber(payload.stock_quantity) ??
      parseNumber(payload.inventory_level);
    ...
    if (targetQty === null || parseNumber(payload.item_id) === null) {
      ...
      if (targetQty === null) {
        const delta = parseNumber(payload.delta);
        if (delta === null) {
          throw new Error("Magento inventory_adjust requires qty/stock_quantity/inventory_level or delta");
        }
...
function buildMagentoProductBody(payload: Record<string, unknown>): Record<string, unknown> {
  const hasProduct = typeof payload.product === "object" && payload.product !== null && !Array.isArray(payload.product);
...
function normalizeBaseUrl(baseUrl: string): string {
  ...
  if (trimmed.startsWith("http://")) return `https://${trimmed.slice("http://".length).replace(/\/+$/, "")}`;
...
async function parseApiResponse(provider: string, response: Response): Promise<Record<string, unknown>> {
  const text = await response.text();
  let parsed: unknown = {};
