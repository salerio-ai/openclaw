...
const DEFAULT_GATEWAY_URL = "ws://127.0.0.1:17999";
...
function buildLabel(beacon: GatewayBonjourBeacon): string {
  ...
  const port = beacon.port ?? beacon.gatewayPort ?? 17999;
...
export async function promptRemoteGatewayConfig(
  ...
  if (selectedBeacon) {
    ...
    const port = selectedBeacon.port ?? selectedBeacon.gatewayPort ?? 17999;
    if (host) {
      ...
      if (mode === "direct") {
        ...
        await prompter.note(
          [
            ...
            `Using: ${suggestedUrl}`,
            "If your gateway is loopback-only, choose SSH tunnel and keep ws://127.0.0.1:17999.",
          ].join("\n"),
          "Direct remote",
        );
      } else {
