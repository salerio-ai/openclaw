...
vi.mock("../infra/ports.js", () => ({
  ...
  formatPortDiagnostics: () => ["Port 17999 is already in use."],
}));
...
describe("daemon-cli coverage", () => {
  ...
  it("installs the daemon when requested", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "17999"], {
      from: "user",
    });
  ...
  it("installs the daemon with json output", async () => {
    ...
    await program.parseAsync(["daemon", "install", "--port", "17999", "--json"], {
      from: "user",
    });
