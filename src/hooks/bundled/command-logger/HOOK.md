...
2. **Appends to log file** - Writes a JSON line to `~/.bustly/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.bustly/logs/commands.log ~/.bustly/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openclaw`:
   ```
   /home/username/.bustly/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
