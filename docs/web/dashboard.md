...
- **Localhost**: open `http://127.0.0.1:17999/`. If you see “unauthorized,” run `openclaw dashboard` and use the tokenized link (`?token=...`).
...
- Ensure the gateway is reachable (local: `openclaw status`; remote: SSH tunnel `ssh -N -L 17999:127.0.0.1:17999 user@host` then open `http://127.0.0.1:17999/?token=...`).
