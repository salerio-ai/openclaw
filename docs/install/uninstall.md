...
```bash
rm -rf "${OPENCLAW_STATE_DIR:-$HOME/.bustly}"
```
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), repeat step 3 for each state dir (defaults are `~/.bustly-<profile>`).
...
Remove-Item -Force "$env:USERPROFILE\.bustly\gateway.cmd"
```

If you used a profile, delete the matching task name and `~\.bustly-<profile>\gateway.cmd`.
