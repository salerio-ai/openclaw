...
- `--dev`: isolate state under `~/.bustly-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.bustly-<name>`.
...
- `--workspace <dir>`: agent workspace path (default `~/.bustly/workspace`).
...
- `node run --host <gateway-host> --port 17999`
- `node status`
