...
- Jobs persist under `~/.bustly/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.bustly/cron/jobs.json` by default.
...
- Job store: `~/.bustly/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.bustly/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
