...
`~/.bustly/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
- Index storage: per-agent SQLite at `~/.bustly/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
...
- Session logs live on disk (`~/.bustly/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
