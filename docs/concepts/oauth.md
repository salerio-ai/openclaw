...
- Runtime cache (managed automatically; don’t edit): `~/.bustly/agents/<agentId>/agent/auth.json`

Legacy import-only file (still supported, but not the main store):

- `~/.bustly/credentials/oauth.json` (imported into `auth-profiles.json` on first use)
