...
- Secrets live in `~/.bustly/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.bustly/agent/auth-profiles.json`).
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.bustly/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.bustly/agents/<agentId>/agent/auth-profiles.json` under `profiles`.
