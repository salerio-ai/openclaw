...
read_when:
  - Designing workspace memory (~/.bustly/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.bustly/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.bustly/workspace` as canonical human-readable memory.
...
```
~/.bustly/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.bustly/workspace`:
