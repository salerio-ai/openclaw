...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.bustly/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.bustly/node.json` (node id + pairing token).
...
Path: `~/.bustly/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.bustly/exec-approvals.sock",
...
- Unix socket at `~/.bustly/exec-approvals.sock` (0600).
