...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenClaw calls the same
     login endpoint, stores the access token in `~/.bustly/credentials/matrix/credentials.json`,
...
`~/.bustly/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
