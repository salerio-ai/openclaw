...
sequenceDiagram
    ...
    participant 用户
    participant 桌面端 as openclaude 桌面端
    ...
    桌面端->>桌面端: 生成 login_trace_id / device_id / machine_id
    桌面端->>浏览器: 打开登录链接（redirect_url）
    浏览器->>网页端: 访问登录页 /login
    用户->>网页端: 完成登录（账号 / OAuth）

    %% ===== 用户存在性判断 =====
    网页端->>业务服务: 查询用户是否存在
    alt 新用户
        业务服务->>网页端: 进入 Onboarding 流程
        用户->>网页端: 选择行业 / 头像 / 名称
        用户->>网页端: Connect 数据源
        用户->>网页端: 订阅方案 & 绑卡
        业务服务->>业务服务: 创建默认 Workspace
    else 已有用户
        业务服务->>业务服务: 查询用户下的 Workspace
        业务服务->>业务服务: 获取最新 Workspace
    end

    %% ===== 授权与回调 =====
    网页端->>认证服务: 请求授权（绑定 user + workspace）
    认证服务->>认证服务: 生成 auth_code
    认证服务->>桌面端: 回调本地地址\n/authorize?code=auth_code

    %% ===== 换取 Token =====
    桌面端->>认证服务: 使用 auth_code 换取 Token
    认证服务->>桌面端: access_token + workspace_id + skills

    %% ===== 写入本地配置 =====
    桌面端->>桌面文件: 写入 Token 与 Workspace 配置
    桌面端->>桌面端: 登录完成，Skill 可用
```
