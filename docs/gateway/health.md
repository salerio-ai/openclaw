...
- Creds on disk: `ls -l ~/.bustly/credentials/whatsapp/<accountId>/creds.json` (mtime should be recent).
- Session store: `ls -l ~/.bustly/agents/<agentId>/sessions/sessions.json` (path can be overridden in config). Count and recent recipients are surfaced via `status`.
...
- Gateway unreachable → start it: `openclaw gateway --port 17999` (use `--force` if the port is busy).
