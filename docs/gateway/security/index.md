...
- Tighten local perms (`~/.bustly` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
- **WhatsApp**: `~/.bustly/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.bustly/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.bustly/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.bustly/credentials/oauth.json`
...
OpenClaw stores session transcripts on disk under `~/.bustly/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.bustly` (see the audit section below). If you need
...
- If you install plugins from npm (`openclaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.bustly/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.bustly/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.bustly or your logs.”
...
- `~/.bustly/openclaw.json`: `600` (user read/write only)
- `~/.bustly`: `700` (user only)
...
Assume anything under `~/.bustly/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.bustly/sandboxes`
...
2. Review the relevant transcript(s): `~/.bustly/agents/<agentId>/sessions/*.jsonl`.
