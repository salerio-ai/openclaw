...
- **Gateway WS (control plane)**: the WebSocket endpoint on `127.0.0.1:17999` by default; can be bound to LAN/tailnet via `gateway.bind`.
- **Direct WS transport**: a LAN/tailnet-facing Gateway WS endpoint (no SSH).
- **SSH transport (fallback)**: remote control by forwarding `127.0.0.1:17999` over SSH.
...
- TXT keys (non-secret):
  ...
  - `gatewayPort=17999` (Gateway WS + HTTP)
...
- `gateway.bind` in `~/.bustly/openclaw.json` controls the Gateway bind mode.
